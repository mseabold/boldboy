...
uint8_t IoJoypad::readAddr(uint16_t addr) {
    uint8_t val = mSelection;
    if(addr == IOREG_P1) {
        // Remember: all these pins are active low!
        if(!(mSelection & IOREG_P1_OUT_P14)) {
            val |= (mMask & JOYPAD_BMASK_P14_MASK) ^ JOYPAD_BMASK_P14_MASK;
        }

        if(!(mSelection & IOREG_P1_OUT_P15)) {
            val |= ((mMask & JOYPAD_BMASK_P15_MASK) ^ JOYPAD_BMASK_P15_MASK) >> JOYPAD_BMASK_P15_SHIFT;
        }
        return val;
    }
    return 0xff;
