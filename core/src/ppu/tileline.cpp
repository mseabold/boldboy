...
TileLine::TileLine(uint8_t *tileData, Pixel::PixelType pixelType) {
    load(tileData, pixelType);
}
...
void TileLine::load(uint8_t *tileData, Pixel::PixelType pixelType) {
    ...
    for(int8_t pixIdx = 7; pixIdx >= 0; --pixIdx) {
        ...
        mPixels[pixIdx] = Pixel(pix, pixelType);
...
TileLine TileLine::empty() {
    uint8_t data[2] = {0,0};
    return TileLine(data, Pixel::ptBackground);
}
